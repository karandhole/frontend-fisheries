  ...
  return (
    <Box sx={{  pt: 5,backgroundColor:'white' }}>
      ...
      {Array.from({length:3}).map(()=>(
         ...
         ))} {t("fisheriesFoundation")} {Array.from({length:3}).map(()=>(
          <Typography component='img' height={12} src={star} />
         ))}
      </Typography>
      <Box
    ...
    '::before': {
      ...
      borderRadius:5
            ...
            <Typography textAlign='center' sx={{position:'relative',zIndex:2}} color='white' variant="body1">{t("fisheriesaboutDescription")}</Typography>
        </Box>
      </Box>
    </Box>
  );
